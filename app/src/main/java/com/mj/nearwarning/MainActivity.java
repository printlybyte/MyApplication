...
private  LocationManager locationManager;
    ...
    private void initView() {
        ...
        int size = locationManager.getAllProviders().size();
        if (size > 0) {
            Log.i("QWEQWE", "获取到的支持信息大小是：" + size);
            for (int i = 0; i < size; i++) {
                Log.i("QWEQWE", "获取到的支持信息是" + locationManager.getAllProviders().get(i));
            }
        }
        ...
        locationManager.requestLocationUpdates(LocationManager.GPS_PROVIDER, 3000, 8, new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                Log.i("QWEQWE", "位置改变" + location.getLatitude());
                Log.i("QWEQWE", "获取到的支持信息大小是：" + location.getLatitude() + "==" + location.getLongitude());
            }
            ...
            public void onProviderEnabled(String provider) {
                Log.i("QWEQWE", "Gps可用" + locationManager.getLastKnownLocation(provider).getLatitude());
            }
